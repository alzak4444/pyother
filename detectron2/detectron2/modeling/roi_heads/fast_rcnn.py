...
#        num_fg = fg_inds.nonzero().numel()
        num_fg = torch.where(fg_inds)[0].numel()

        fg_gt_classes = self.gt_classes[fg_inds]
        fg_pred_classes = pred_classes[fg_inds]

        num_false_negative = torch.where(fg_pred_classes == bg_class_ind)[0].numel()
        num_accurate = torch.where(pred_classes == self.gt_classes)[0].numel()
        fg_num_accurate = torch.where(fg_pred_classes == fg_gt_classes)[0].numel()
